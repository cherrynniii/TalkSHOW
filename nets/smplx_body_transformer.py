...
from nets.spg.transformer_autoregressive import GatedPixelTransformer as pixeltransformer
...
class TrainWrapper(TrainWrapperBaseClass):
    ...
    def __init__(self, args, config):
        ...
        if self.convert_to_6d:
            ...
            dim, layer = 256, 15
        self.generator = pixeltransformer(2048, dim, layer, self.num_classes, self.audio, self.bh_model).to(self.device)
    ...
    def __call__(self, bat):
        ...
        # print(logits.shape) # [128, 22, 2, 2048]
        # print(latents.shape)    # [128, 22, 2]

        log = logits[:, :-1, :, :]
        tgt = latents[:, 1:, :].long()

        # print(log.shape) # [128, 22, 2, 2048]
        # print(tgt.shape)    # [128, 22, 2]

        loss = F.cross_entropy(log.contiguous().view(-1, logits.shape[-1]), tgt.contiguous().view(-1))
        loss.backward()
