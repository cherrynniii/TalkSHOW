...
from nets.spg.s2g_simple_mlp import Generator as s2g_simple_mlp
...
class TrainWrapper(TrainWrapperBaseClass):
    ...
    def get_loss(self,
        ...
        jaw_loss = torch.mean((pred_poses[:, :, b_j:b_e] - gt_poses[:, :, b_j:b_e]) ** 2)
        body_loss = torch.mean((pred_poses[:, :, b_e:b_h] - gt_poses[:, :, b_e:b_h]) ** 2)
        hand_loss = torch.mean((pred_poses[:, :, b_h:b_f] - gt_poses[:, :, b_h:b_f]) ** 2)

        if self.expression:
            exp_loss = torch.mean((pred_poses[:, :, b_f:] - gt_poses[:, :, b_f:]) ** 2)
        else:
            exp_loss = torch.tensor(0.0, device=pred_poses.device)
        gen_loss = jaw_loss + body_loss + hand_loss + exp_loss
