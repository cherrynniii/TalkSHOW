...
class Generator(nn.Module):
    def __init__(self,
        ...
        # 트랜스포머 인코더 레이어 정의
        for mid_dim, out_ch in decoder_configs:
            ...
            decoder = TransformerDecoder(decoder_layer, num_layers=3)
            self.decoder.append(decoder)
            self.final_out.append(nn.Linear(out_dim, out_ch))
    ...
    def forward(self, in_spec, gt_poses=None, id=None, pre_state=None, time_steps=None):
        ...
        memory = feature.permute(0, 2, 1)  # 여기서 permute (1, 150, 256)
        ...
        for i in range(self.decoder.__len__()):
            # 수정
            transout = self.decoder[i](tgt=tgt, memory=memory, tgt_mask=causal_mask)
